...
logging.basicConfig(filename='agente_error.log', level=logging.ERROR, format='%(asctime)s %(levelname)s: %(message)s')
...
def send_zpl_to_printer(printer_name, zpl_content):
    try:
        ...
        if not zpl_str.strip():
            print('   [!] Error: Contenido ZPL recibido para la impresora está vacío.')
            return False
...
def load_or_create_config():
    if os.path.exists(CONFIG_FILE):
        try:
            with open(CONFIG_FILE, 'r', encoding='utf-8') as f:
                ...
                print(f'    * Cola: {cfg.get("queue_code")}')
    ...
    print('===================================================')
    server_url = input('1. URL del Servidor [https://tracker360.mywire.org]: ').strip() or 'https://tracker360.mywire.org'
    api_key = input('2. Clave API de Sistema (Header X-API-Key): ').strip()
    queue_code = input('3. Codigo de Sector / Cola [RECEPCION]: ').strip().upper() or 'RECEPCION'
    ...
    if printers:
        ...
        if choice.isdigit() and 1 <= int(choice) <= len(printers):
            ...
            printer_name = choice
    if not printer_name:
        printer_name = input('4. Nombre exacto de la impresora: ').strip()
    cfg = {'server_url': server_url.rstrip('/'), 'api_key': api_key, 'queue_code': queue_code, 'printer_name': printer_name}
...
def run_agent():
    ...
    print('--> Escuchando trabajos de impresión... (Ctrl + C para salir)')
    print('---------------------------------------------------\n')
    while True:
        try:
